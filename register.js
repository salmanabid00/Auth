...
const pageError = pageParams.get("error");

if (pageError === "email_exists") {
  showBanner("This email is already registered!", "error");
} else if (pageError === "username_taken") {
  showBanner("Username already taken! Please choose another.", "error");
}

document.getElementById("name").addEventListener("input", function () {
  if (this.value.trim().length >= 3) showValid("name", "nameError");
  else showError("name", "nameError");
});

document.getElementById("email").addEventListener("input", function () {
  if (isValidEmail(this.value.trim())) showValid("email", "emailError");
...
document.getElementById("username").addEventListener("input", function () {
  ...
  else showError("username", "usernameError");
});
...
document.getElementById("password").addEventListener("input", function () {
  ...
  if (isStrongPassword(val)) showValid("password", "passwordError");
  else showError("password", "passwordError");

  const confirm = document.getElementById("confirmPassword").value;
  if (confirm.length > 0) {

    if (confirm === val) showValid("confirmPassword", "confirmError");
    else showError("confirmPassword", "confirmError");
  }
});
...
document.getElementById("confirmPassword")
  ...
  document.getElementById("registerForm").addEventListener("submit", function (event) {
    ...
    if (localStorage.getItem("email_" + email)) {
      window.location.href = "Register.html?error=email_exists";
      return;
    }
    
    if (localStorage.getItem("user_" + username)) {
      window.location.href = "Register.html?error=username_taken";
      return;
    }
    ...
    window.location.href = `login.html?registered=true&name=${encodeURIComponent(name)}`;
  });
